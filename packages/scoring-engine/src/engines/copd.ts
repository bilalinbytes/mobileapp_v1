...
export function runCopdAlertEngine(
  log: DailyLogInput,
  ...
  if (copd.haemoptysis_volume === "cup" || copd.haemoptysis_volume === "massive") {
    return buildAlert(
      "red",
      "Action recommended: hemoptysis volume reached at least one cup today.",
      metricWithSeverity(
        { haemoptysis_volume: copd.haemoptysis_volume },
        RED_SEVERITY.immediate,
      ),
      "copd-red-haemoptysis-cup",
    );
  }
  ...
  if (typeof log.spo2_rest === "number" && log.spo2_rest <= 80) {
    return buildAlert(
      ...
      "copd-red-spo2-immediate",
    );
  }

  const hasPurulentLargePattern =
    (copd.sputum_colour === "yellow" || copd.sputum_colour === "green") &&
    copd.sputum_volume === "large_amount";

  if (hasPurulentLargePattern) {
    return buildAlert(
      "red",
      "Action recommended: Purulent sputum with increased volume and worsening breathlessness.",
      metricWithSeverity({
        sputum_colour: copd.sputum_colour,
        sputum_volume: copd.sputum_volume,
        mmrc_today: log.mmrc_today,
      }, RED_SEVERITY.exacerbation),
      "copd-red-exacerbation-pattern",
    );
  }
  ...
  if (typeof oxygenIncrease === "number" && oxygenIncrease >= 3) {
    return buildAlert(
      "red",
      "Action recommended: oxygen requirement increased by >=3 L/min above baseline.",
      metricWithSeverity({
      ...
      }, RED_SEVERITY.oxygenIncrease),
      "copd-red-oxygen-increase",
    );
  }
  ...
  if (
    ...
    return buildAlert(
      "red",
      "Action recommended: resting SpO2 <=85% for 2 consecutive days.",
      metricWithSeverity({
  ...
  if (
    ...
    return buildAlert(
      "red",
      "Action recommended: energy level <4/10 for 2 consecutive days.",
      metricWithSeverity({
  ...
  if (
    ...
    return buildAlert(
      "red",
      "Action recommended: sleep disturbed by breathing for 2 consecutive days.",
      metricWithSeverity({
  ...
  if (
    ...
    return buildAlert(
      "red",
      "Action recommended: chest heaviness VAS >=8/10 for 2 consecutive days.",
      metricWithSeverity({
  ...
  if (
    ...
    return buildAlert(
      "red",
      "Action recommended: fever >=102F for 2 consecutive days.",
      metricWithSeverity({
