...
export function runIldAlertEngine(
  log: DailyLogInput,
  ...
  if (log.haemoptysis === true) {
    return buildAlert(
      "red",
      "Hemoptysis reported.",
      metricWithSeverity({ haemoptysis: true }, RED_SEVERITY.immediate),
      "ild-red-haemoptysis",
    );
  }
  ...
  if (typeof log.heart_rate === "number" && log.heart_rate >= 160) {
    return buildAlert(
      "red",
      "Heart rate >=160 bpm.",
      metricWithSeverity({ heart_rate: log.heart_rate }, RED_SEVERITY.immediate),
      "ild-red-heart-rate",
    );
  }
  ...
  if (
    ...
    return buildAlert(
      "red",
      "Resting SpO2 <=85% for 2 consecutive days.",
      metricWithSeverity({
  ...
  if (
    ...
    return buildAlert(
      "red",
      "SpO2 dropped >=6% from baseline for 2 consecutive days.",
      metricWithSeverity({
        baseline_spo2: baseline.baseline_spo2,
        spo2_rest_today: log.spo2_rest,
      }, RED_SEVERITY.sustained),
      "ild-red-spo2-drop-2d",
    );
  }

  const exertionalBaseline =
    baseline.baseline_exertional_spo2 ?? baseline.baseline_spo2;
  if (
    hasConsecutiveDayMatch(log, previousLogs, (entry: DailyLogInput) => {
      const drop = numberDropFromBaseline(
        entry.spo2_exertion,
    ...
    return buildAlert(
      "red",
      "Exertional SpO2 dropped >=10% from baseline for 2 consecutive days.",
      metricWithSeverity({
        baseline_exertional_spo2: exertionalBaseline,
        spo2_exertion_today: log.spo2_exertion,
      }, RED_SEVERITY.sustained),
      "ild-red-exertional-drop-2d",
    );
  }
  ...
  if (
    ...
    return buildAlert(
      "red",
      "mMRC increased by >=2 grades for 2 consecutive days.",
      metricWithSeverity({
  ...
  if (
    ...
    return buildAlert(
      "red",
      "Fever with severe worsening cough for 2 consecutive days.",
      metricWithSeverity({
  ...
  if (typeof oxygenIncrease === "number" && oxygenIncrease >= 3) {
    return buildAlert(
      "red",
      "Oxygen requirement increased by >=3 L/min.",
      metricWithSeverity(
        {
        ...
        RED_SEVERITY.highReliability,
      ),
      "ild-red-oxygen-increase",
    );
  }
  ...
  const moderateSymptoms = ["breathlessness", "cough", "fatigue"].some((key) => {
    const value = getSymptomValue(
      log,
      key as "breathlessness" | "cough" | "fatigue",
    );
  ...
  const previousKbild =
    typeof ild.kbild_previous === "number"
      ? ild.kbild_previous
      : previousIld?.kbild_score ?? null;
  if (
    typeof ild.kbild_score === "number" &&
