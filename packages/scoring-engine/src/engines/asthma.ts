...
export function runAsthmaAlertEngine(
  log: DailyLogInput,
  ...
  const control =
    asthma.asthma_control_status ??
    classifyAsthmaControl(asthma.asthma_control_responses);

  if (log.haemoptysis === true) {
    return buildAlert(
      "red",
      "Action recommended: hemoptysis reported.",
      metricWithSeverity(
        { haemoptysis: true, asthma_control: control },
        RED_SEVERITY.immediate,
      ),
      "asthma-red-haemoptysis",
    );
  }
  ...
  if (typeof chestPain === "number" && chestPain >= 8) {
    return buildAlert(
      ...
      metricWithSeverity(
        { chest_pain_vas: chestPain, asthma_control: control },
        RED_SEVERITY.immediate,
      ),
      "asthma-red-chest-pain",
    );
  }
  ...
  if (typeof log.spo2_rest === "number" && log.spo2_rest <= 80) {
    return buildAlert(
      ...
      metricWithSeverity(
        { spo2_rest: log.spo2_rest, asthma_control: control },
        RED_SEVERITY.immediate,
      ),
      "asthma-red-spo2-immediate",
    );
  }

  if (
    typeof asthma.rescue_inhaler_puffs === "number" &&
    ...
    return buildAlert(
      "red",
      `Action recommended: Rescue inhaler use >=8 puffs today${controlPhrase}.`,
      metricWithSeverity(
        {
  ...
  if (
    ...
    return buildAlert(
      "red",
      "Action recommended: poorly controlled asthma for 2 consecutive days.",
      metricWithSeverity(
        { asthma_control_today: control, asthma_control_yesterday: previousControl },
        RED_SEVERITY.controlPattern,
      ),
      "asthma-red-poor-control-2d",
    );
  }
  ...
  if (
    hasConsecutiveDayMatch(
      ...
      previousLogs,
      (entry: DailyLogInput) =>
        typeof entry.spo2_rest === "number" && entry.spo2_rest <= 88,
    )
  ) {
    return buildAlert(
      "red",
      "Action recommended: resting SpO2 <=88% for 2 consecutive days.",
      metricWithSeverity({
        spo2_rest_today: log.spo2_rest,
        spo2_rest_yesterday: previousLogs[0]?.spo2_rest ?? null,
        asthma_control: control,
      }, RED_SEVERITY.sustained),
      "asthma-red-spo2-2d",
    );
  }
  ...
  if (
    ...
    return buildAlert(
      "red",
      "Action recommended: SpO2 dropped >=4% from baseline for 2 consecutive days.",
      metricWithSeverity({
  ...
  if (
    ...
    return buildAlert(
      "red",
      "Action recommended: exertional SpO2 dropped >=10% from baseline for 2 consecutive days.",
      metricWithSeverity({
        baseline_spo2: baseline.baseline_spo2,
  ...
  if (
    ...
    return buildAlert(
      "red",
      "Action recommended: mMRC increased by >=3 grades for 2 consecutive days.",
      metricWithSeverity({
  ...
  if (
    hasConsecutiveDayMatch(log, previousLogs, (entry: DailyLogInput) => {
      ...
      const coughIncrease = numberIncreaseFromBaseline(
        cough,
        baseline.baseline_cough_vas,
      );
    ...
    return buildAlert(
      "red",
      "Action recommended: fever with severe worsening cough for 2 consecutive days.",
      metricWithSeverity({
  ...
  const moderateSymptoms = ["breathlessness", "cough", "wheeze", "fatigue"].some(
    (key) => {
      const value = getSymptomValue(
        log,
        key as "breathlessness" | "cough" | "wheeze" | "fatigue",
      );
  ...
  const sideEffectsReported =
    Array.isArray(log.side_effects)
  ...
  return buildAlert(
    "green",
    "Asthma stable and well controlled.",
    {
      asthma_control: control,
      yellow_score: yellowScore,
      rescue_inhaler_puffs: asthma.rescue_inhaler_puffs,
