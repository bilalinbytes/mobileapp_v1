...
import {
  buildAlert,
  getBronchiectasisData,
  getSymptomValue,
  hasConsecutiveDayMatch,
  hasThreeDayMatch,
  numberDropFromBaseline,
  numberIncreaseFromBaseline,
  oxygenIncreaseFromBaseline,
} from "../utils/baseline";
...
function getTopTwoReasons(reasons: Array<{ label: string; points: number }>): string {
  return reasons
...
export function runBronchiectasisAlertEngine(
  log: DailyLogInput,
  ...
  if (bronch.haemoptysis_volume === "glass" || bronch.haemoptysis_volume === "massive") {
    return buildAlert(
      "red",
      "Action recommended: hemoptysis volume reached at least one glass today.",
      metricWithSeverity(
        { haemoptysis_volume: bronch.haemoptysis_volume },
        RED_SEVERITY.immediate,
      ),
      "bronch-red-haemoptysis-glass",
    );
  }
  ...
  if (typeof chestPain === "number" && chestPain >= 8) {
    return buildAlert(
      "red",
      "Action recommended: severe chest pain VAS >=8/10.",
      metricWithSeverity({ chest_pain_vas: chestPain }, RED_SEVERITY.immediate),
      "bronch-red-chest-pain",
    );
  }
  ...
  if (typeof log.spo2_rest === "number" && log.spo2_rest <= 80) {
    return buildAlert(
      "red",
      "Action recommended: resting SpO2 <=80%.",
      metricWithSeverity({ spo2_rest: log.spo2_rest }, RED_SEVERITY.immediate),
      "bronch-red-spo2-immediate",
    );
  }

  // Hard rule: single-day sputum colour or volume change NEVER triggers RED.
  const persistentDarkGreenSputum = hasThreeDayMatch(log, previousLogs, (entry: DailyLogInput) => {
    const data = getBronchiectasisData(entry);
    return (
      data.sputum_colour === "dark_green" &&
  ...
  if (persistentDarkGreenSputum) {
    return buildAlert(
      "red",
      "Action recommended: dark green sputum with one cup or more volume for 3 consecutive days, suggestive of infection.",
      metricWithSeverity({
        sputum_colour: bronch.sputum_colour,
        sputum_volume: bronch.sputum_volume,
      }, RED_SEVERITY.persistentSputum),
      "bronch-red-sputum-3d",
    );
  }

  if (
    hasConsecutiveDayMatch(log, previousLogs, (entry: DailyLogInput) => {
      return typeof entry.temperature_f === "number" && entry.temperature_f >= 102;
    })
  ) {
    return buildAlert(
      "red",
      "Action recommended: temperature >=102F for 2 consecutive days.",
      metricWithSeverity({
        temperature_f_today: log.temperature_f,
        temperature_f_yesterday: previousLogs[0]?.temperature_f ?? null,
      }, RED_SEVERITY.systemic),
      "bronch-red-fever-2d",
    );
  }
  ...
  if (
    hasConsecutiveDayMatch(log, previousLogs, (entry: DailyLogInput) => {
      const data = getBronchiectasisData(entry);
      return (
        typeof entry.temperature_f === "number" &&
    ...
    return buildAlert(
      "red",
      "Action recommended: fever with severe malaise for 2 consecutive days.",
      metricWithSeverity({
        temperature_f_today: log.temperature_f,
        malaise_today: bronch.malaise,
      }, RED_SEVERITY.systemic),
      "bronch-red-fever-malaise-2d",
    );
  }
  ...
  if (
    hasConsecutiveDayMatch(
      ...
      previousLogs,
      (entry: DailyLogInput) =>
        typeof entry.spo2_rest === "number" && entry.spo2_rest <= 85,
    )
  ) {
    return buildAlert(
      "red",
      "Action recommended: resting SpO2 <=85% for 2 consecutive days with respiratory deterioration.",
      metricWithSeverity({
        spo2_rest_today: log.spo2_rest,
        spo2_rest_yesterday: previousLogs[0]?.spo2_rest ?? null,
      }, RED_SEVERITY.respiratory),
      "bronch-red-spo2-2d",
    );
  }
  ...
  if (
    hasConsecutiveDayMatch(log, previousLogs, (entry: DailyLogInput) => {
      const drop = numberDropFromBaseline(
        entry.spo2_rest,
    ...
    return buildAlert(
      "red",
      "Action recommended: SpO2 dropped >=6% from baseline for 2 consecutive days.",
      metricWithSeverity({
        baseline_spo2: baseline.baseline_spo2,
        spo2_rest_today: log.spo2_rest,
      }, RED_SEVERITY.respiratory),
      "bronch-red-spo2-drop-2d",
    );
  }
  ...
  if (
    ...
    return buildAlert(
      "red",
      "Action recommended: oxygen requirement increased by >=3 L/min for 2 consecutive days.",
      metricWithSeverity({
        oxygen_requirement_litres: log.oxygen_requirement_litres,
        baseline_oxygen_litres: baseline.baseline_oxygen_litres,
      }, RED_SEVERITY.respiratory),
      "bronch-red-oxygen-2d",
    );
  }

  if (
    hasConsecutiveDayMatch(log, previousLogs, (entry: DailyLogInput) => {
      const increase = numberIncreaseFromBaseline(
        entry.mmrc_today,
        baseline.baseline_mmrc,
      );
    ...
    return buildAlert(
      "red",
      "Action recommended: mMRC increased by >=2 grades for 2 consecutive days.",
      metricWithSeverity({
  ...
  if (
    ...
    return buildAlert(
      "red",
      "Action recommended: pedal edema present for 2 consecutive days.",
      metricWithSeverity(
        { pedal_oedema: bronch.pedal_oedema },
        RED_SEVERITY.respiratory,
      ),
      "bronch-red-oedema-2d",
    );
  }
  ...
  if (
    ...
    return buildAlert(
      "red",
      "Action recommended: wheezing with symptom worsening persisted for two consecutive days.",
      metricWithSeverity({
        wheezing_today: bronch.wheezing,
        symptom_vas_today: log.vas_symptoms,
      }, RED_SEVERITY.respiratory),
      "bronch-red-wheeze-worsening-2d",
    );
  }

  const yellowReasons: Array<{ label: string; points: number }> = [];
  let yellowScore = 0;

  if (
    bronch.sputum_colour === "pale_yellow" ||
  ...
  if (
    typeof log.temperature_f === "number" &&
  ...
  const mmrcIncrease = numberIncreaseFromBaseline(
    log.mmrc_today,
    baseline.baseline_mmrc,
  );
  ...
  if (yellowScore >= 4) {
    return buildAlert(
      "yellow",
      `Review suggested: ${getTopTwoReasons(yellowReasons)}.`,
      {
  ...
  return buildAlert(
    "green",
    "Stable. Continue airway clearance and maintenance therapy.",
    {
      yellow_score: yellowScore,
