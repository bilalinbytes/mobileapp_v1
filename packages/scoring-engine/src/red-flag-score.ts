...
export function computeRedFlagScore(
  log: DailyLogInput,
  ...
  if (diseaseAutoTrigger) {
    return createAutoTriggeredResult(
      diseaseAutoTrigger.score,
      diseaseAutoTrigger.reason,
    );
  }
  ...
  if (baseline.effective_dashboard === "ild") {
    ...
    const spo2Drop = numberDropFromBaseline(
      log.spo2_rest,
...
function getImmediateAutoTrigger(log: DailyLogInput): string | null {
...
function getDiseaseAutoTrigger(
  log: DailyLogInput,
  baseline: PatientBaseline,
...
function createAutoTriggeredResult(
  score: number,
  reason: string,
): RedFlagScoreResult {
